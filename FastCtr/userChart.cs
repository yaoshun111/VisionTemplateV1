...
namespace FastCtr
{
    public partial class UserChart : UserControl
    {
        ...
        bool Flag = false;
        static object lock_txt = new object();
        ...
        private string filePath = new DirectoryInfo("../../../ConfigurationData/ChartPara/Set.txt").FullName;


        private List<int>[] listX = new List<int>[8];
        ...
        public List<int>[] ListX { set { listX = value; } get { return listX; } }
        [Category("数据点绑定")]
        ...
        public void InitChartset()
        {
            ...
            {
                FileSystemWatcher fsw = new FileSystemWatcher(Path.GetDirectoryName(filePath), "*.txt");
                fsw.EnableRaisingEvents = true;
                fsw.NotifyFilter = NotifyFilters.LastWrite;
                fsw.Changed += new FileSystemEventHandler(fsw_Changed);
            }
            ...
            {
                SaveStatic.SaveTxt(filePath, "", false);
                FileSystemWatcher fsw = new FileSystemWatcher(Path.GetDirectoryName(filePath), "*.txt");
                fsw.EnableRaisingEvents = true;
                fsw.NotifyFilter = NotifyFilters.LastWrite;
                fsw.Changed += new FileSystemEventHandler(fsw_Changed);
        ...
        private static string[][] ReadMultiTxtC(string _path)
        {
            lock (lock_txt)
            {
        ...
        private static void SaveTxtC(string _path, IEnumerable<string[]> _contents, bool _append)
        {
            lock (lock_txt)
            {
                string str = "";
        ...
        public void fresh()
        {
            if (Flag)
            {
                ...
                if (File.Exists(filePath))
                {
                    try
                    {
                        string[][] aa = ReadMultiTxtC(filePath);
                    ...
                    {
                        MessageBox.Show("读取图表配置文件失败！"+exp);
                    }
                }
                else
                {
            ...
            if (listdatabinPath[0] != "")
            {
                ...
                string[] _d = SaveStatic.ReadTxt(listdatabinPath[0]);
                if (_d == null)
                {
                    return;
                }
            ...
            {
                string[] _d = SaveStatic.ReadTxt(listdatabinPath[1]);
                if (_d == null)
                {
                    return;
                }
            ...
            {
                string[] _d = SaveStatic.ReadTxt(listdatabinPath[2]);
                if (_d == null)
                {
                    return;
                }
            ...
            {
                string[] _d = SaveStatic.ReadTxt(listdatabinPath[3]);
                if (_d == null)
                {
                    return;
                }
            ...
            {
                string[] _d = SaveStatic.ReadTxt(listdatabinPath[4]);
                if (_d == null)
                {
                    return;
                }
            ...
            {
                string[] _d = SaveStatic.ReadTxt(listdatabinPath[5]);
                if (_d == null)
                {
                    return;
                }
            ...
            {
                string[] _d = SaveStatic.ReadTxt(listdatabinPath[6]);
                if (_d == null)
                {
                    return;
                }
            ...
            {
                string[] _d = SaveStatic.ReadTxt(listdatabinPath[7]);
                if (_d == null)
                {
                    return;
                }
